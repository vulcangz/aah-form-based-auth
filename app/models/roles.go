...
type Role struct {
	...
	Quota     null.Float32 `boil:"quota" json:"quota,omitempty" toml:"quota" yaml:"quota,omitempty"`
...
var RoleWhere = struct {
	...
	Quota     whereHelpernull_Float32
}{
	ID:        whereHelperuint64{field: "`roles`.`id`"},
	...
	Quota:     whereHelpernull_Float32{field: "`roles`.`quota`"},
}
...
var (
	roleAllColumns            = []string{"id", "created_at", "updated_at", "deleted_at", "name", "quota"}
	roleColumnsWithoutDefault = []string{"created_at", "updated_at", "deleted_at", "name", "quota"}
...
func (roleL) LoadUsers(e boil.Executor, singular bool, maybeRole interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("`users`.id, `users`.first_name, `users`.last_name, `users`.email, `users`.password, `users`.is_locked, `users`.is_expried, `users`.created_at, `users`.updated_at, `users`.deleted_at, `a`.`role_id`"),
		qm.From("`users`"),
...
func (o *Role) AddUsers(exec boil.Executor, insert bool, related ...*User) error {
	...
	for _, rel := range related {
		query := "insert into `users_roles` (`role_id`, `user_id`) values (?, ?)"
...
func (o *Role) SetUsers(exec boil.Executor, insert bool, related ...*User) error {
	query := "delete from `users_roles` where `role_id` = ?"
...
func (o *Role) RemoveUsers(exec boil.Executor, related ...*User) error {
	...
	var err error
	query := fmt.Sprintf(
		"delete from `users_roles` where `role_id` = ? and `user_id` in (%s)",
...
func FindRole(exec boil.Executor, iD uint64, selectCols ...string) (*Role, error) {
	roleObj := &Role{}
...
func RoleExists(exec boil.Executor, iD uint64) (bool, error) {
	var exists bool
